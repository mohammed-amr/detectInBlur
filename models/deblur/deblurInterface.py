import models.deblur.model as model
import models.deblur.dataCommon as  dataCommon
...
class Deblurer():
    ...
    def deblurImage(self, image):
        image, pad_width = dataCommon.pad(np.asarray(image), divisor=2**(self.args.n_scales-1))   # pad in case of non-divisible size
        imgs = [image]
        if self.args.gaussian_pyramid:
            imgs = dataCommon.generate_pyramid(*imgs, n_scales=self.args.n_scales)

        imgs = dataCommon.np2tensor(*imgs)
        blur = imgs[0]

        inputToNetwork = [blur, blur, pad_width, 0, ""]
        netInput, target = dataCommon.to(
                inputToNetwork[0], inputToNetwork[1], device=self.device, dtype=self.dtype_eval)

        for pyramidIn, imagePyramid in enumerate(netInput):
            netInput[pyramidIn] = torch.unsqueeze(imagePyramid, 0)

        output = self.model(netInput)

        output[0], _ = dataCommon.pad(output[0], pad_width=pad_width, negative=True)
...
def getArgs(model_location):
    ...
    argString = '--save_dir ' + model_location +  ' --demo true --demo_input_dir blurSweep --demo_output_dir output'
    ...
    if args.start_epoch < 0: # start from scratch or continue from the last epoch
        ...
        if os.path.exists(model_dir):
            ...
            for name in model_list:
                epoch_number = int(re.findall('\\d+', name)[0]) # model example name model-100.pt
                if last_epoch < epoch_number:
