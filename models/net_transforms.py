...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self, min_size, max_size, image_mean, image_std, crop_images = False, training = True, normalize_images = True):
    ...
    def forward(self,
        ...
        for i in range(len(images)):
            ...
            if self.normalize_images:
                if newMeans is not None:
                    newMean = newMeans[i, :]
                    newSTD = newSTDs[i, :]
                    image = self.normalize(image, newMean, newSTD)
                else:
                    image = self.normalize(image, self.image_mean, self.image_std)
    ...
    def min_by_axis(self, the_list):
        # type: (List[List[int]]) -> List[int]
        mins = the_list[0]
        for sublist in the_list[1:]:
            for index, item in enumerate(sublist):
                mins[index] = min(mins[index], item)
        return mins
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        if self.crop_images:
            min_size = self.min_by_axis([list(img.shape) for img in images])
            stride = float(size_divisible)
            min_size = list(min_size)
            min_size[1] = int(math.floor(float(min_size[1]) / stride) * stride)
            min_size[2] = int(math.floor(float(min_size[2]) / stride) * stride)

            batch_shape = [len(images)] + min_size
            batched_imgs = images[0].new_full(batch_shape, 0)
            for img, crop_img in zip(images, batched_imgs):
                crop_img.copy_(img[: crop_img.shape[0], : crop_img.shape[1], : crop_img.shape[2]])
        else:
